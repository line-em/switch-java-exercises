...
public class dSoft1 {
	// Question 02
	public static double getBouquetPrice(int roses, int tulips, double roseValue,
													 double tulipValue) {
	...
	// Question 06
	public static double getBuildingHeightWithTales(double humanHeight,
																	double humanShadow,
																	double buildingShadow) {
	...
	public static int getTimeInSeconds(int hours, int minutes, int seconds) {
		if (hours < 0 || minutes < 0 || seconds < 0) return -1; //Should be at least 0
		return (hours * 3600) + (minutes * 60) + seconds;
	}

	public static double getMoveSpeed(double totalDistance, int hours, int minutes,
												 int seconds) {
		...
		else return (totalDistance / totalTime);
	}

	public static double getRunDistance(double moveSpeed, int hours, int minutes,
													int seconds) {
		if (moveSpeed < 0) return -1;
		if (moveSpeed == 0) return 0; //Stayed in place, so 0m.

		int totalTime = getTimeInSeconds(hours, minutes, seconds);

		if (totalTime < 0) return -1;
		else return (moveSpeed * totalTime) / 1000;
	}
